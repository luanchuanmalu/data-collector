...
class Cleaner():
    ...
    def clean(self, data, targetInfo):
        ...
        # use most common
        return (sameData, anyDiffData, allDiffData, oneDiffData, twoDiffData)
    ...
    def doCompare(self, secID, beginDate, endDate):
        ...
        for targetName, targetInfo in self.targetsSetting.items():
            print '--------------%s-----------' %(targetName)
            (sameData, anyDiffData, allDiffData, oneDiffData, twoDiffData) = self.compareTarget(secID, beginDate, endDate, targetInfo)
            # print diffData
            self.saveToMysql(oneDiffData, 'comparedata','onediffdata')
            self.saveToMysql(twoDiffData, 'comparedata','twoDiffData')
